...
def lambert_illumination(model_vertexes, fv_array, light_dir, back_face_culling, z_buffer, texture_v, texture_faces,
                         image, img):
    distance = light_dir / np.linalg.norm(light_dir)
    z_buff = np.full((800, 800), -np.inf)
    width = image.size[0]
    height = image.size[1]
    pix = image.load()
                    ...
                    for y in range(min_point[1], max_point[1]):
                        for x in range(min_point[0], max_point[0]):
                            ...
                            if bar_coords[0] < 0 or bar_coords[1] < 0 or bar_coords[2] < 0:
                                ...
                                z = bar_coords[0] * frst_point[2] + bar_coords[1] * sec_point[2] + bar_coords[2] * \
                                    thrd_point[2]
                                if z > z_buff[800 - y - 1, 800 - x - 1]:
                                    u = (bar_coords[0] * frst_texture[0] + bar_coords[1] * sec_texture[0] + bar_coords[
                                        2] * thrd_texture[0]) * width
                                    v = (bar_coords[0] * frst_texture[1] + bar_coords[1] * sec_texture[1] + bar_coords[
                                        2] * thrd_texture[1]) * height
                                    r = pix[width - u, height - v][0]
                                    g = pix[width - u, height - v][1]
                                    b = pix[width - u, height - v][2]
                                    img[800 - y - 1, 800 - x - 1] = 0.8 * 0.5 * diffuse * np.array([r, g, b],
                                                                                            dtype=np.float64)
                                    z_buff[800 - y - 1, 800 - x - 1] = z
                            else:
                                continue
                else:
